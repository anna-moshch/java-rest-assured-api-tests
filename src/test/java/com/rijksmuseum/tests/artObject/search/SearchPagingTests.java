...
public class SearchPagingTests extends AbstractTest {
    ...
    public void verifyMaximumPageParam() {
        List<ArtObjectsItem> actualResult = steps.search.getArtObjectItems(data.params.getParamPagingMax());
    ...
    public void verifyingPsParamMoreThanAllowed() {
        List<ArtObjectsItem> actualResult = steps.search.getArtObjectItems(data.params.getParamPsMoreThanAllowed());
    ...
    public void verifyPageParamMoreThanAllowed() {
        List<ArtObjectsItem> actualResult = steps.search.getArtObjectItems(data.params.getParamPagingMoreThanAllowed());
        assertThat(actualResult.size()).isEqualTo(0);
    }

    @Test(description = "TC_04 Verifying that the results are returned for incorrect page number")
    public void verifyIncorrectPageNumber() {
        List<ArtObjectsItem> actualResult = steps.search.getArtObjectItems(data.params.getParamIncorrectPageNumber());
